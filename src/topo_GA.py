...
""""""""""""""""""""""""""""""""""""""""""""""""""
"""""""""""""""""""""""""""""""GA"""""""""""""""""""""""""""""""""""""
from GA_calc import GA

target=j*1.5
def objective(x):
    err=np.sqrt(np.square(x - target))
    print(f"To target:{target},Err:{err}")
    return err
def construction():
    pass
...
def generate_cauchy_random(op):
    cauchy = onp.random.randint(
        low=op["bound_low"][op["paras_at"][0],op["paras_at"][0]],  # 坐标的下界
        high=op["bound_up"][op["paras_at"][0],op["paras_at"][0]],  # 坐标的上界
        size=(op["sites_num"], 2)  # 生成 n*2 的矩阵
    ).flatten()
    return cauchy

GA(optimizationParams2, generate_cauchy_random, objective, construction, load=np.array([0., -200.]),
   location_fns=location_fns, fixed_location=fixed_location)
