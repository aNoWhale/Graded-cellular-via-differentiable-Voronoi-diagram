...
def linear_fem(Nx, Ny, Lx, Ly, optimizationParams, p, filename, load:np.array, location_fns, fixed_location, **kwargs):
    # Weak forms.
    class LinearElasticity(Problem):
        ...
        def set_voronoi(self, op, p,epoch):
            thetas = generate_voronoi_separate(op, p,epoch=epoch)
    ...
    thetas=problem.set_voronoi(optimizationParams,p,epoch=kwargs["gen"] if "gen" in kwargs.keys() else 0)
...
if __name__ == '__main__':
    ...
    val=linear_fem(Nx, Ny, Lx, Ly, optimizationParams, p, f"x_", load=np.array([100., 0.]),
                   location_fns=load_location_x, fixed_location=fixed_location_x)
    print(val)