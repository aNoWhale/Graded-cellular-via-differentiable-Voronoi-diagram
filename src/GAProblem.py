...
def FitnessCalculationFunction(individual,gen):
    ...
    x=linear_fem(Nx, Ny, Lx, Ly, optimizationParams, individual,f"{gen}/x_{gen}_{ri}", load=np.array([100.,0.]),
                 location_fns=load_location_x, fixed_location=fixed_location_x)
    y=linear_fem(Nx, Ny, Lx, Ly, optimizationParams, individual,f"{gen}/y_{gen}_{ri}", load=np.array([0.,100.]),
                 location_fns=load_location_y, fixed_location=fixed_location_y)
    return x,y
